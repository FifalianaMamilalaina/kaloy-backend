  ...
  >
    <div class="card-body p-4">
      ...
      <h2 class="card-title text-base font-bold text-primary mb-2 truncate">
        <span> #{{ eventData['id'] }} - Event </span>
      </h2>

      <!-- Corps de la carte : affiche UNIQUEMENT les champs visibles -->
      <div class="space-y-1 text-sm flex-1">
        <template v-for="field in Event.getAllSearchFieldsMetadata()" :key="field.key">
          <div
          ...
          >
            ...
            <span class="text-right text-base-content truncate max-w-[60%]">
              <template v-if="field.type === 'date'">
                {{ formatDate(eventData[field.key]) }}
              </template>
              <template v-else-if="field.type === 'number'">
                {{ eventData[field.key] != null ? $n(eventData[field.key], 'decimal') : '--' }}
              </template>
              <template v-else-if="field.type === 'select'">
                {{ eventData[field.key] != null ? eventData[field.key].getKeyValue() : '--' }}
              </template>
              <template v-else-if="field.type === 'file'">
                <img
                  v-if="eventData[field.key]"
                  :src="getUrl(eventData[field.key])"
                  :alt="field.key"
...
const eventData = computed(() => {
  return props.event as Record<string, any>
})

const emit = defineEmits<{
  (e: 'request-delete', event: Event): void
  (e: 'request-view', event: Event): void
  (e: 'request-update', event: Event): void
}>()

const deleteRow = () => emit('request-delete', props.event)
const viewRow = () => emit('request-view', props.event)
const updateRow = () => emit('request-update', props.event)
