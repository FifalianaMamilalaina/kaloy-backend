...
const props = defineProps({
  data: { type: Array as PropType<Event[]>, required: true },
  message: String,
...
// Fallback sur EventTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? EventTable,
)

const selectedEntity = ref<Event>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deleteEvent, viewEvent, goToUpdateFormView } = useEvents()
...
const confirmDelete = async () => {
  const { success, error } = await deleteEvent(selectedEntity.value!)
  if (success) {
