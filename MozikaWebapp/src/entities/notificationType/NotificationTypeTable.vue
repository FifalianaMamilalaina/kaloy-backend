<template>
  <table class="table table-auto relative">
    ...
    <tbody class="bg-base-100">
      ...
      <component
        v-else
        v-for="notificationType in data"
        :key="notificationType.getKeyValue()"
        :is="NotificationTypeRow"
...
const visibleColCount = computed(
  () =>
    props.visibleFields.filter((f) =>
      NotificationType.getAllSearchFieldsMetadata().some((m) => m.key === f && m.showInTable),
    ).length,
)
</script>
<style scoped></style>
