<template>
  <table class="table table-auto relative">
    ...
    <tbody class="bg-base-100">
      ...
      <component
        v-else
        v-for="userStatuse in data"
        :key="userStatuse.getKeyValue()"
        :is="UserStatuseRow"
...
import UserStatuseRow from './UserStatuseRow.vue'
...
const visibleColCount = computed(
  () =>
    props.visibleFields.filter((f) =>
      UserStatuse.getAllSearchFieldsMetadata().some((m) => m.key === f && m.showInTable),
    ).length,
)
</script>
<style scoped></style>
