...
import UserStatuseTable from './UserStatuseTable.vue'
import UserStatuseGrid from './UserStatuseGrid.vue'
...
const props = defineProps({
  data: { type: Array as PropType<UserStatuse[]>, required: true },
  message: String,
...
const layoutComponents: Record<string, Component> = {
  list: UserStatuseTable,
  card: UserStatuseGrid,
  // Exemples futurs :
  // kanban: UserStatuseKanban,
  // timeline: UserStatuseTimeline,
}

// Fallback sur UserStatuseTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? UserStatuseTable,
)

const selectedEntity = ref<UserStatuse>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deleteUserStatuse, viewUserStatuse, goToUpdateFormView } = useUserStatuses()
...
const confirmDelete = async () => {
  const { success, error } = await deleteUserStatuse(selectedEntity.value!)
  if (success) {
