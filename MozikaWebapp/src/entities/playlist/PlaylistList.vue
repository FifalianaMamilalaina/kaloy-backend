...
const props = defineProps({
  data: { type: Array as PropType<Playlist[]>, required: true },
  message: String,
...
const layoutComponents: Record<string, Component> = {
  ...
  card: PlaylistGrid,
  // Exemples futurs :
  // kanban: PlaylistKanban,
  // timeline: PlaylistTimeline,
}

// Fallback sur PlaylistTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? PlaylistTable,
)

const selectedEntity = ref<Playlist>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deletePlaylist, viewPlaylist, goToUpdateFormView } = usePlaylists()
...
const confirmDelete = async () => {
  const { success, error } = await deletePlaylist(selectedEntity.value!)
  if (success) {
