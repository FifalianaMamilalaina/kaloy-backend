...
import DownloadGrid from './DownloadGrid.vue'
...
const props = defineProps({
  data: { type: Array as PropType<Download[]>, required: true },
  message: String,
...
const layoutComponents: Record<string, Component> = {
  ...
  card: DownloadGrid,
  // Exemples futurs :
  // kanban: DownloadKanban,
  // timeline: DownloadTimeline,
}

// Fallback sur DownloadTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? DownloadTable,
)

const selectedEntity = ref<Download>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deleteDownload, viewDownload, goToUpdateFormView } = useDownloads()
...
const confirmDelete = async () => {
  const { success, error } = await deleteDownload(selectedEntity.value!)
  if (success) {
