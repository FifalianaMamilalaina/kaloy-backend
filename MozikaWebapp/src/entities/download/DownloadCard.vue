  ...
  >
    <div class="card-body p-4">
      ...
      <h2 class="card-title text-base font-bold text-primary mb-2 truncate">
        <span> #{{ downloadData['id'] }} - Download </span>
      </h2>

      <!-- Corps de la carte : affiche UNIQUEMENT les champs visibles -->
      <div class="space-y-1 text-sm flex-1">
        <template v-for="field in Download.getAllSearchFieldsMetadata()" :key="field.key">
          <div
          ...
          >
            ...
            <span class="text-right text-base-content truncate max-w-[60%]">
              <template v-if="field.type === 'date'">
                {{ formatDate(downloadData[field.key]) }}
              </template>
              <template v-else-if="field.type === 'number'">
                {{
                  downloadData[field.key] != null ? $n(downloadData[field.key], 'decimal') : '--'
                }}
              </template>
              <template v-else-if="field.type === 'select'">
                {{ downloadData[field.key] != null ? downloadData[field.key].getKeyValue() : '--' }}
              </template>
              <template v-else-if="field.type === 'file'">
                <img
                  v-if="downloadData[field.key]"
                  :src="getUrl(downloadData[field.key])"
                  :alt="field.key"
                  class="max-h-10 max-w-10"
                />
              </template>
              <template v-else>
                {{ downloadData[field.key] ?? '--' }}
              </template>
            </span>
          </div>
        </template>
...
const downloadData = computed(() => {
  return props.download as Record<string, any>
})

const emit = defineEmits<{
  (e: 'request-delete', download: Download): void
  (e: 'request-view', download: Download): void
  (e: 'request-update', download: Download): void
}>()

const deleteRow = () => emit('request-delete', props.download)
const viewRow = () => emit('request-view', props.download)
const updateRow = () => emit('request-update', props.download)
