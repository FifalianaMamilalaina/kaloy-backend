<template>
  <table class="table table-auto relative">
    ...
    <tbody class="bg-base-100">
      ...
      <component
        v-else
        v-for="participationStatuse in data"
        :key="participationStatuse.getKeyValue()"
        :is="ParticipationStatuseRow"
...
import ParticipationStatuseRow from './ParticipationStatuseRow.vue'
...
const visibleColCount = computed(
  () =>
    props.visibleFields.filter((f) =>
      ParticipationStatuse.getAllSearchFieldsMetadata().some((m) => m.key === f && m.showInTable),
    ).length,
)
</script>
<style scoped></style>
