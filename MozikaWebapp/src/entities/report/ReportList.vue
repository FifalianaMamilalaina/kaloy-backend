...
const props = defineProps({
  data: { type: Array as PropType<Report[]>, required: true },
  message: String,
...
// Fallback sur ReportTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? ReportTable,
)

const selectedEntity = ref<Report>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deleteReport, viewReport, goToUpdateFormView } = useReports()
...
const confirmDelete = async () => {
  const { success, error } = await deleteReport(selectedEntity.value!)
  if (success) {
