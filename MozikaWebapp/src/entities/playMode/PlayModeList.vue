...
import PlayModeTable from './PlayModeTable.vue'
import PlayModeGrid from './PlayModeGrid.vue'
...
const props = defineProps({
  data: { type: Array as PropType<PlayMode[]>, required: true },
  message: String,
...
const layoutComponents: Record<string, Component> = {
  ...
  card: PlayModeGrid,
  // Exemples futurs :
  // kanban: PlayModeKanban,
  // timeline: PlayModeTimeline,
}

// Fallback sur PlayModeTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? PlayModeTable,
)

const selectedEntity = ref<PlayMode>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deletePlayMode, viewPlayMode, goToUpdateFormView } = usePlayModes()
...
const confirmDelete = async () => {
  const { success, error } = await deletePlayMode(selectedEntity.value!)
  if (success) {
