<template>
  <table class="table table-auto relative">
    ...
    <tbody class="bg-base-100">
      ...
      <component
        v-else
        v-for="verificationChannel in data"
        :key="verificationChannel.getKeyValue()"
        :is="VerificationChannelRow"
...
import VerificationChannelRow from './VerificationChannelRow.vue'
...
const visibleColCount = computed(
  () =>
    props.visibleFields.filter((f) =>
      VerificationChannel.getAllSearchFieldsMetadata().some((m) => m.key === f && m.showInTable),
    ).length,
)
</script>
<style scoped></style>
