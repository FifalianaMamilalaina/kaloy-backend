...
const props = defineProps({
  data: { type: Array as PropType<Album[]>, required: true },
  message: String,
...
// Fallback sur AlbumTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? AlbumTable,
)

const selectedEntity = ref<Album>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deleteAlbum, viewAlbum, goToUpdateFormView } = useAlbums()
...
const confirmDelete = async () => {
  const { success, error } = await deleteAlbum(selectedEntity.value!)
  if (success) {
