<template>
  <table class="table table-auto relative">
    ...
    <tbody class="bg-base-100">
      ...
      <component
        v-else
        v-for="contentSubmission in data"
        :key="contentSubmission.getKeyValue()"
        :is="ContentSubmissionRow"
...
import ContentSubmissionRow from './ContentSubmissionRow.vue'
...
const visibleColCount = computed(
  () =>
    props.visibleFields.filter((f) =>
      ContentSubmission.getAllSearchFieldsMetadata().some((m) => m.key === f && m.showInTable),
    ).length,
)
</script>
<style scoped></style>
