<template>
  <table class="table table-auto relative">
    ...
    <tbody class="bg-base-100">
      ...
      <component
        v-else
        v-for="verificationCode in data"
        :key="verificationCode.getKeyValue()"
        :is="VerificationCodeRow"
...
import VerificationCodeRow from './VerificationCodeRow.vue'
...
const visibleColCount = computed(
  () =>
    props.visibleFields.filter((f) =>
      VerificationCode.getAllSearchFieldsMetadata().some((m) => m.key === f && m.showInTable),
    ).length,
)
</script>
<style scoped></style>
