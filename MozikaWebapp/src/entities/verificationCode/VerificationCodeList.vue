<template>
  ...
  <component
    ...
    @request-view="viewVerificationCode"
...
import VerificationCodeTable from './VerificationCodeTable.vue'
import VerificationCodeGrid from './VerificationCodeGrid.vue'
...
const props = defineProps({
  data: { type: Array as PropType<VerificationCode[]>, required: true },
  message: String,
...
const layoutComponents: Record<string, Component> = {
  list: VerificationCodeTable,
  card: VerificationCodeGrid,
  // Exemples futurs :
  // kanban: VerificationCodeKanban,
  // timeline: VerificationCodeTimeline,
}

// Fallback sur VerificationCodeTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? VerificationCodeTable,
)

const selectedEntity = ref<VerificationCode>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deleteVerificationCode, viewVerificationCode, goToUpdateFormView } = useVerificationCodes()
...
const confirmDelete = async () => {
  const { success, error } = await deleteVerificationCode(selectedEntity.value!)
  if (success) {
