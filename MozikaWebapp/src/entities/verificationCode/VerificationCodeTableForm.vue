<template>
  <div class="w-full bg-base-100 border rounded border-base-300 p-4">
    <h3 class="text-lg font-semibold mb-4 text-gray-700">Verification code</h3>
      ...
      <table class="table w-full">
        <thead>
          <tr class="text-primary-focus">
            <th class="">#</th>
            <th class="">Channelid verification channels</th>
            <th class="">Destination</th>
            <th class="">Code</th>
            <th class="">Expires at</th>
            <th class="">Consumed at</th>
        ...
        <tbody>
          <template v-for="(row, index) in rows" :key="row._internalId">
            <VerificationCodeRowForm
              :internalId="index + 1"
              :verificationCode="row.data"
...
import VerificationCodeRowForm from '@/entities/verificationCode/VerificationCodeRowForm.vue'
import GenesisInput from '@/components/form/GenesisInput.vue'
import PlusIcon from '@/components/icons/PlusIcon.vue'

const rowToAdd = ref(10)
const props = defineProps<{
  getTableDataCallback: (data: VerificationCodeFormDTO[]) => void
  initialData?: VerificationCode[]
  violations?: Record<string, string>[]
}>()

const { rows, addRow, removeRow, getTableData } = useTableForm<
  VerificationCode,
  VerificationCodeFormDTO
>(props.initialData)
