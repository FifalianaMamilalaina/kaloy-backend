<template>
  <div class="w-full bg-base-100 border rounded border-base-300 p-4">
    <h3 class="text-lg font-semibold mb-4 text-gray-700">Notification preference</h3>
      ...
      <table class="table w-full">
        <thead>
          <tr class="text-primary-focus">
            <th class="">#</th>
            <th class="">Notificationtypeid notification types</th>
            <th class="">Is enabled</th>
            <th class=""></th>
          </tr>
        </thead>
        <tbody>
          <template v-for="(row, index) in rows" :key="row._internalId">
            <NotificationPreferenceRowForm
              :internalId="index + 1"
              :notificationPreference="row.data"
...
import NotificationPreferenceRowForm from '@/entities/notificationPreference/NotificationPreferenceRowForm.vue'
import GenesisInput from '@/components/form/GenesisInput.vue'
import PlusIcon from '@/components/icons/PlusIcon.vue'

const rowToAdd = ref(10)
const props = defineProps<{
  getTableDataCallback: (data: NotificationPreferenceFormDTO[]) => void
