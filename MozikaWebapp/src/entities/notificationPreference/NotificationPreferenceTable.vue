<template>
  <table class="table table-auto relative">
    ...
    <tbody class="bg-base-100">
      ...
      <component
        v-else
        v-for="notificationPreference in data"
        :key="notificationPreference.getKeyValue()"
        :is="NotificationPreferenceRow"
...
import NotificationPreferenceRow from './NotificationPreferenceRow.vue'
...
const visibleColCount = computed(
  () =>
    props.visibleFields.filter((f) =>
      NotificationPreference.getAllSearchFieldsMetadata().some((m) => m.key === f && m.showInTable),
    ).length,
)
</script>
<style scoped></style>
