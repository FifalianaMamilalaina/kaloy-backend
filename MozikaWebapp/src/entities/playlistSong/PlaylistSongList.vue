...
import PlaylistSongTable from './PlaylistSongTable.vue'
...
const layoutComponents: Record<string, Component> = {
  list: PlaylistSongTable,
  card: PlaylistSongGrid,
  // Exemples futurs :
  // kanban: PlaylistSongKanban,
  // timeline: PlaylistSongTimeline,
}

// Fallback sur PlaylistSongTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? PlaylistSongTable,
)

const selectedEntity = ref<PlaylistSong>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deletePlaylistSong, viewPlaylistSong, goToUpdateFormView } = usePlaylistSongs()
...
const confirmDelete = async () => {
  const { success, error } = await deletePlaylistSong(selectedEntity.value!)
  if (success) {
