  ...
  >
    <div class="card-body p-4">
      ...
      <h2 class="card-title text-base font-bold text-primary mb-2 truncate">
        <span> #{{ venueData['id'] }} - Venue </span>
      </h2>

      <!-- Corps de la carte : affiche UNIQUEMENT les champs visibles -->
      <div class="space-y-1 text-sm flex-1">
        <template v-for="field in Venue.getAllSearchFieldsMetadata()" :key="field.key">
          <div
          ...
          >
            ...
            <span class="text-right text-base-content truncate max-w-[60%]">
              <template v-if="field.type === 'date'">
                {{ formatDate(venueData[field.key]) }}
              </template>
              <template v-else-if="field.type === 'number'">
                {{ venueData[field.key] != null ? $n(venueData[field.key], 'decimal') : '--' }}
              </template>
              <template v-else-if="field.type === 'select'">
                {{ venueData[field.key] != null ? venueData[field.key].getKeyValue() : '--' }}
              </template>
              <template v-else-if="field.type === 'file'">
                <img
                  v-if="venueData[field.key]"
                  :src="getUrl(venueData[field.key])"
                  :alt="field.key"
...
const venueData = computed(() => {
  return props.venue as Record<string, any>
})

const emit = defineEmits<{
  (e: 'request-delete', venue: Venue): void
  (e: 'request-view', venue: Venue): void
  (e: 'request-update', venue: Venue): void
}>()

const deleteRow = () => emit('request-delete', props.venue)
const viewRow = () => emit('request-view', props.venue)
const updateRow = () => emit('request-update', props.venue)
