<template>
  <table class="table table-auto relative">
    ...
    <tbody class="bg-base-100">
      ...
      <component
        v-else
        v-for="audioStorageType in data"
        :key="audioStorageType.getKeyValue()"
        :is="AudioStorageTypeRow"
...
import AudioStorageTypeRow from './AudioStorageTypeRow.vue'
...
const visibleColCount = computed(
  () =>
    props.visibleFields.filter((f) =>
      AudioStorageType.getAllSearchFieldsMetadata().some((m) => m.key === f && m.showInTable),
    ).length,
)
</script>
<style scoped></style>
