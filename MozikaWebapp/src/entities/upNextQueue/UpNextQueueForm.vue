...
import { ref } from 'vue'
import { UpNextQueue, UpNextQueueFormDTO } from '@/models/UpNextQueueModel.ts'
...
const emit = defineEmits<{
  (e: 'submit', payload: Partial<UpNextQueueFormDTO>): void
  (e: 'cancel', payload: Partial<UpNextQueueFormDTO>): void
}>()

const formModel = ref<Partial<UpNextQueueFormDTO>>({
  ...UpNextQueueFormDTO.parse(props.upNextQueue),
})
const useridUsersSearchField = UpNextQueue.getSearchFieldByKey('useridUsers')
const useridUsersDefaultValue = props.upNextQueue
  ? (props.upNextQueue?.useridUsers?.getKeyValue?.() ?? undefined)
  : undefined
const songidSongsSearchField = UpNextQueue.getSearchFieldByKey('songidSongs')
const songidSongsDefaultValue = props.upNextQueue
  ? (props.upNextQueue?.songidSongs?.getKeyValue?.() ?? undefined)
  : undefined
