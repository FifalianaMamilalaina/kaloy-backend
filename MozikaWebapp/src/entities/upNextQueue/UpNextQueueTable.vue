<template>
  <table class="table table-auto relative">
    ...
    <tbody class="bg-base-100">
      ...
      <component
        v-else
        v-for="upNextQueue in data"
        :key="upNextQueue.getKeyValue()"
        :is="UpNextQueueRow"
...
import UpNextQueueRow from './UpNextQueueRow.vue'
...
const visibleColCount = computed(
  () =>
    props.visibleFields.filter((f) =>
      UpNextQueue.getAllSearchFieldsMetadata().some((m) => m.key === f && m.showInTable),
    ).length,
)
</script>
<style scoped></style>
