<template>
  <div class="w-full bg-base-100 border rounded border-base-300 p-4">
    <h3 class="text-lg font-semibold mb-4 text-gray-700">Up next queue</h3>
...
import UpNextQueueRowForm from '@/entities/upNextQueue/UpNextQueueRowForm.vue'
import GenesisInput from '@/components/form/GenesisInput.vue'
import PlusIcon from '@/components/icons/PlusIcon.vue'

const rowToAdd = ref(10)
const props = defineProps<{
  getTableDataCallback: (data: UpNextQueueFormDTO[]) => void
  initialData?: UpNextQueue[]
  violations?: Record<string, string>[]
}>()

const { rows, addRow, removeRow, getTableData } = useTableForm<UpNextQueue, UpNextQueueFormDTO>(
  props.initialData,
)
