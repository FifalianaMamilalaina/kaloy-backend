<template>
  <div class="w-full bg-base-100 border rounded border-base-300 p-4">
    <h3 class="text-lg font-semibold mb-4 text-gray-700">Notification</h3>
      ...
      <table class="table w-full">
        <thead>
          <tr class="text-primary-focus">
            <th class="">#</th>
            <th class="">Typeid notification types</th>
            <th class="">Content</th>
            <th class="">Relatedentitytypeid interaction targets</th>
            <th class="">Related entity id</th>
            <th class="">Is read</th>
...
import NotificationRowForm from '@/entities/notification/NotificationRowForm.vue'
import GenesisInput from '@/components/form/GenesisInput.vue'
import PlusIcon from '@/components/icons/PlusIcon.vue'

const rowToAdd = ref(10)
const props = defineProps<{
  getTableDataCallback: (data: NotificationFormDTO[]) => void
  initialData?: Notification[]
  violations?: Record<string, string>[]
}>()

const { rows, addRow, removeRow, getTableData } = useTableForm<Notification, NotificationFormDTO>(
  props.initialData,
)
