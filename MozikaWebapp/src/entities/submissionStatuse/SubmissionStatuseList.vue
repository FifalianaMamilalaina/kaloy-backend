<template>
  ...
  <component
    ...
    @request-view="viewSubmissionStatuse"
...
import SubmissionStatuseTable from './SubmissionStatuseTable.vue'
import SubmissionStatuseGrid from './SubmissionStatuseGrid.vue'
...
const props = defineProps({
  data: { type: Array as PropType<SubmissionStatuse[]>, required: true },
  message: String,
...
const layoutComponents: Record<string, Component> = {
  list: SubmissionStatuseTable,
  card: SubmissionStatuseGrid,
  // Exemples futurs :
  // kanban: SubmissionStatuseKanban,
  // timeline: SubmissionStatuseTimeline,
}

// Fallback sur SubmissionStatuseTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? SubmissionStatuseTable,
)

const selectedEntity = ref<SubmissionStatuse>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deleteSubmissionStatuse, viewSubmissionStatuse, goToUpdateFormView } =
...
const confirmDelete = async () => {
  const { success, error } = await deleteSubmissionStatuse(selectedEntity.value!)
  if (success) {
