<template>
  <table class="table table-auto relative">
    ...
    <tbody class="bg-base-100">
      ...
      <component
        v-else
        v-for="submissionStatuse in data"
        :key="submissionStatuse.getKeyValue()"
        :is="SubmissionStatuseRow"
...
import SubmissionStatuseRow from './SubmissionStatuseRow.vue'
...
const visibleColCount = computed(
  () =>
    props.visibleFields.filter((f) =>
      SubmissionStatuse.getAllSearchFieldsMetadata().some((m) => m.key === f && m.showInTable),
    ).length,
)
</script>
<style scoped></style>
