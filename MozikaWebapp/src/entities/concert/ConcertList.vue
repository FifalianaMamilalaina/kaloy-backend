...
import ConcertGrid from './ConcertGrid.vue'
...
const layoutComponents: Record<string, Component> = {
  ...
  card: ConcertGrid,
  // Exemples futurs :
  // kanban: ConcertKanban,
  // timeline: ConcertTimeline,
}

// Fallback sur ConcertTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? ConcertTable,
)

const selectedEntity = ref<Concert>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deleteConcert, viewConcert, goToUpdateFormView } = useConcerts()
...
const confirmDelete = async () => {
  const { success, error } = await deleteConcert(selectedEntity.value!)
  if (success) {
