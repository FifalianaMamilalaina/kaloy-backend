<template>
  ...
  <component
    ...
    @request-view="viewReportStatuse"
...
import ReportStatuseTable from './ReportStatuseTable.vue'
import ReportStatuseGrid from './ReportStatuseGrid.vue'
...
const props = defineProps({
  data: { type: Array as PropType<ReportStatuse[]>, required: true },
  message: String,
...
const layoutComponents: Record<string, Component> = {
  list: ReportStatuseTable,
  card: ReportStatuseGrid,
  // Exemples futurs :
  // kanban: ReportStatuseKanban,
  // timeline: ReportStatuseTimeline,
}

// Fallback sur ReportStatuseTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? ReportStatuseTable,
)

const selectedEntity = ref<ReportStatuse>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deleteReportStatuse, viewReportStatuse, goToUpdateFormView } = useReportStatuses()
...
const confirmDelete = async () => {
  const { success, error } = await deleteReportStatuse(selectedEntity.value!)
  if (success) {
