<template>
  <table class="table table-auto relative">
    ...
    <tbody class="bg-base-100">
      ...
      <component
        v-else
        v-for="reportStatuse in data"
        :key="reportStatuse.getKeyValue()"
        :is="ReportStatuseRow"
...
import ReportStatuseRow from './ReportStatuseRow.vue'
...
const visibleColCount = computed(
  () =>
    props.visibleFields.filter((f) =>
      ReportStatuse.getAllSearchFieldsMetadata().some((m) => m.key === f && m.showInTable),
    ).length,
)
</script>
<style scoped></style>
