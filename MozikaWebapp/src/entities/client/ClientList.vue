...
const props = defineProps({
  data: { type: Array as PropType<Client[]>, required: true },
  message: String,
...
// Fallback sur ClientTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? ClientTable,
)

const selectedEntity = ref<Client>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deleteClient, viewClient, goToUpdateFormView } = useClients()
...
const confirmDelete = async () => {
  const { success, error } = await deleteClient(selectedEntity.value!)
  if (success) {
