...
import EditorialPlaylistSongRow from './EditorialPlaylistSongRow.vue'
...
const visibleColCount = computed(
  () =>
    props.visibleFields.filter((f) =>
      EditorialPlaylistSong.getAllSearchFieldsMetadata().some((m) => m.key === f && m.showInTable),
    ).length,
)
</script>
<style scoped></style>
