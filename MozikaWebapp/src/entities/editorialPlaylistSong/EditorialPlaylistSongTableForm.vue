<template>
  <div class="w-full bg-base-100 border rounded border-base-300 p-4">
    <h3 class="text-lg font-semibold mb-4 text-gray-700">Editorial playlist song</h3>
      ...
      <table class="table w-full">
        <thead>
          <tr class="text-primary-focus">
            <th class="">#</th>
            <th class="">Editorialplaylistid editorial playlists</th>
        ...
        <tbody>
          <template v-for="(row, index) in rows" :key="row._internalId">
            <EditorialPlaylistSongRowForm
              :internalId="index + 1"
              :editorialPlaylistSong="row.data"
...
import EditorialPlaylistSongRowForm from '@/entities/editorialPlaylistSong/EditorialPlaylistSongRowForm.vue'
import GenesisInput from '@/components/form/GenesisInput.vue'
import PlusIcon from '@/components/icons/PlusIcon.vue'

const rowToAdd = ref(10)
const props = defineProps<{
  getTableDataCallback: (data: EditorialPlaylistSongFormDTO[]) => void
  initialData?: EditorialPlaylistSong[]
