<template>
  ...
  <component
    ...
    @request-view="viewEditorialPlaylistSong"
...
import EditorialPlaylistSongTable from './EditorialPlaylistSongTable.vue'
import EditorialPlaylistSongGrid from './EditorialPlaylistSongGrid.vue'
...
const layoutComponents: Record<string, Component> = {
  list: EditorialPlaylistSongTable,
  card: EditorialPlaylistSongGrid,
  // Exemples futurs :
  // kanban: EditorialPlaylistSongKanban,
  // timeline: EditorialPlaylistSongTimeline,
}

// Fallback sur EditorialPlaylistSongTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? EditorialPlaylistSongTable,
)

const selectedEntity = ref<EditorialPlaylistSong>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deleteEditorialPlaylistSong, viewEditorialPlaylistSong, goToUpdateFormView } =
...
const confirmDelete = async () => {
  const { success, error } = await deleteEditorialPlaylistSong(selectedEntity.value!)
  if (success) {
