...
import FollowGrid from './FollowGrid.vue'
...
// Fallback sur FollowTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? FollowTable,
)

const selectedEntity = ref<Follow>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deleteFollow, viewFollow, goToUpdateFormView } = useFollows()
...
const confirmDelete = async () => {
  const { success, error } = await deleteFollow(selectedEntity.value!)
  if (success) {
