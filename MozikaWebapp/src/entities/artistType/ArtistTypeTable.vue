<template>
  <table class="table table-auto relative">
    ...
    <tbody class="bg-base-100">
      ...
      <component
        v-else
        v-for="artistType in data"
        :key="artistType.getKeyValue()"
        :is="ArtistTypeRow"
...
import ArtistTypeRow from './ArtistTypeRow.vue'
...
const visibleColCount = computed(
  () =>
    props.visibleFields.filter((f) =>
      ArtistType.getAllSearchFieldsMetadata().some((m) => m.key === f && m.showInTable),
    ).length,
)
</script>
<style scoped></style>
