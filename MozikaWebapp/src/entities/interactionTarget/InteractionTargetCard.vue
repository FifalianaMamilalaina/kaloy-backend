  ...
  >
    <div class="card-body p-4">
      ...
      <h2 class="card-title text-base font-bold text-primary mb-2 truncate">
        <span> #{{ interactionTargetData['id'] }} - InteractionTarget </span>
      </h2>
          ...
          >
            ...
            <span class="text-right text-base-content truncate max-w-[60%]">
              <template v-if="field.type === 'date'">
                {{ formatDate(interactionTargetData[field.key]) }}
              </template>
              <template v-else-if="field.type === 'number'">
                {{
                  interactionTargetData[field.key] != null
                    ? $n(interactionTargetData[field.key], 'decimal')
                    : '--'
                }}
              </template>
              <template v-else-if="field.type === 'select'">
                {{
                  interactionTargetData[field.key] != null
                    ? interactionTargetData[field.key].getKeyValue()
                    : '--'
                }}
              </template>
              <template v-else-if="field.type === 'file'">
                <img
                  v-if="interactionTargetData[field.key]"
                  :src="getUrl(interactionTargetData[field.key])"
                  :alt="field.key"
                  class="max-h-10 max-w-10"
                />
              </template>
              <template v-else>
                {{ interactionTargetData[field.key] ?? '--' }}
              </template>
            </span>
          </div>
        </template>
...
const interactionTargetData = computed(() => {
  return props.interactionTarget as Record<string, any>
})
...
const deleteRow = () => emit('request-delete', props.interactionTarget)
const viewRow = () => emit('request-view', props.interactionTarget)
const updateRow = () => emit('request-update', props.interactionTarget)
