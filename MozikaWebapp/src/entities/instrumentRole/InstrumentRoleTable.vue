<template>
  <table class="table table-auto relative">
    ...
    <tbody class="bg-base-100">
      ...
      <component
        v-else
        v-for="instrumentRole in data"
        :key="instrumentRole.getKeyValue()"
        :is="InstrumentRoleRow"
...
import InstrumentRoleRow from './InstrumentRoleRow.vue'
...
const visibleColCount = computed(
  () =>
    props.visibleFields.filter((f) =>
      InstrumentRole.getAllSearchFieldsMetadata().some((m) => m.key === f && m.showInTable),
    ).length,
)
</script>
<style scoped></style>
