<template>
  ...
  <component
    ...
    @request-view="viewInstrumentRole"
...
import InstrumentRoleTable from './InstrumentRoleTable.vue'
import InstrumentRoleGrid from './InstrumentRoleGrid.vue'
...
const props = defineProps({
  data: { type: Array as PropType<InstrumentRole[]>, required: true },
  message: String,
...
const layoutComponents: Record<string, Component> = {
  list: InstrumentRoleTable,
  card: InstrumentRoleGrid,
  // Exemples futurs :
  // kanban: InstrumentRoleKanban,
  // timeline: InstrumentRoleTimeline,
}

// Fallback sur InstrumentRoleTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? InstrumentRoleTable,
)

const selectedEntity = ref<InstrumentRole>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deleteInstrumentRole, viewInstrumentRole, goToUpdateFormView } = useInstrumentRoles()
...
const confirmDelete = async () => {
  const { success, error } = await deleteInstrumentRole(selectedEntity.value!)
  if (success) {
