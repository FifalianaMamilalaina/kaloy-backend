...
import UserRoleGrid from './UserRoleGrid.vue'
...
const props = defineProps({
  data: { type: Array as PropType<UserRole[]>, required: true },
  message: String,
...
const layoutComponents: Record<string, Component> = {
  ...
  card: UserRoleGrid,
  // Exemples futurs :
  // kanban: UserRoleKanban,
  // timeline: UserRoleTimeline,
}

// Fallback sur UserRoleTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? UserRoleTable,
)

const selectedEntity = ref<UserRole>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deleteUserRole, viewUserRole, goToUpdateFormView } = useUserRoles()
...
const confirmDelete = async () => {
  const { success, error } = await deleteUserRole(selectedEntity.value!)
  if (success) {
