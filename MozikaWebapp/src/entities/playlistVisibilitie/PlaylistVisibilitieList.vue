<template>
  ...
  <component
    ...
    @request-view="viewPlaylistVisibilitie"
...
import PlaylistVisibilitieTable from './PlaylistVisibilitieTable.vue'
import PlaylistVisibilitieGrid from './PlaylistVisibilitieGrid.vue'
...
const layoutComponents: Record<string, Component> = {
  list: PlaylistVisibilitieTable,
  card: PlaylistVisibilitieGrid,
  // Exemples futurs :
  // kanban: PlaylistVisibilitieKanban,
  // timeline: PlaylistVisibilitieTimeline,
}

// Fallback sur PlaylistVisibilitieTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? PlaylistVisibilitieTable,
)

const selectedEntity = ref<PlaylistVisibilitie>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deletePlaylistVisibilitie, viewPlaylistVisibilitie, goToUpdateFormView } =
...
const confirmDelete = async () => {
  const { success, error } = await deletePlaylistVisibilitie(selectedEntity.value!)
  if (success) {
