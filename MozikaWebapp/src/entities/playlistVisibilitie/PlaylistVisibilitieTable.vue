...
import PlaylistVisibilitieRow from './PlaylistVisibilitieRow.vue'
...
const visibleColCount = computed(
  () =>
    props.visibleFields.filter((f) =>
      PlaylistVisibilitie.getAllSearchFieldsMetadata().some((m) => m.key === f && m.showInTable),
    ).length,
)
</script>
<style scoped></style>
