  ...
  >
    <div class="card-body p-4">
      ...
      <h2 class="card-title text-base font-bold text-primary mb-2 truncate">
        <span> #{{ playlistVisibilitieData['id'] }} - PlaylistVisibilitie </span>
      </h2>

      <!-- Corps de la carte : affiche UNIQUEMENT les champs visibles -->
      <div class="space-y-1 text-sm flex-1">
        <template
          v-for="field in PlaylistVisibilitie.getAllSearchFieldsMetadata()"
          :key="field.key"
        >
          <div
          ...
          >
            ...
            <span class="text-right text-base-content truncate max-w-[60%]">
              <template v-if="field.type === 'date'">
                {{ formatDate(playlistVisibilitieData[field.key]) }}
              </template>
              <template v-else-if="field.type === 'number'">
                {{
                  playlistVisibilitieData[field.key] != null
                    ? $n(playlistVisibilitieData[field.key], 'decimal')
                    : '--'
                }}
              </template>
              <template v-else-if="field.type === 'select'">
                {{
                  playlistVisibilitieData[field.key] != null
                    ? playlistVisibilitieData[field.key].getKeyValue()
                    : '--'
                }}
              </template>
              <template v-else-if="field.type === 'file'">
                <img
                  v-if="playlistVisibilitieData[field.key]"
                  :src="getUrl(playlistVisibilitieData[field.key])"
                  :alt="field.key"
                  class="max-h-10 max-w-10"
                />
              </template>
              <template v-else>
                {{ playlistVisibilitieData[field.key] ?? '--' }}
              </template>
            </span>
          </div>
        </template>
...
const playlistVisibilitieData = computed(() => {
  return props.playlistVisibilitie as Record<string, any>
})
...
const deleteRow = () => emit('request-delete', props.playlistVisibilitie)
const viewRow = () => emit('request-view', props.playlistVisibilitie)
const updateRow = () => emit('request-update', props.playlistVisibilitie)
