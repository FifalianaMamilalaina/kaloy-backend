...
import GenreTable from './GenreTable.vue'
import GenreGrid from './GenreGrid.vue'
...
const props = defineProps({
  data: { type: Array as PropType<Genre[]>, required: true },
  message: String,
...
const layoutComponents: Record<string, Component> = {
  ...
  // Exemples futurs :
  // kanban: GenreKanban,
  // timeline: GenreTimeline,
}

// Fallback sur GenreTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? GenreTable,
)

const selectedEntity = ref<Genre>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deleteGenre, viewGenre, goToUpdateFormView } = useGenres()
...
const confirmDelete = async () => {
  const { success, error } = await deleteGenre(selectedEntity.value!)
  if (success) {
