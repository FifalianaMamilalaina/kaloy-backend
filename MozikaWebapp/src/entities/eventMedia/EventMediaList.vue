...
import EventMediaTable from './EventMediaTable.vue'
import EventMediaGrid from './EventMediaGrid.vue'
...
const props = defineProps({
  data: { type: Array as PropType<EventMedia[]>, required: true },
  message: String,
...
const layoutComponents: Record<string, Component> = {
  list: EventMediaTable,
  card: EventMediaGrid,
  // Exemples futurs :
  // kanban: EventMediaKanban,
  // timeline: EventMediaTimeline,
}

// Fallback sur EventMediaTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? EventMediaTable,
)

const selectedEntity = ref<EventMedia>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deleteEventMedia, viewEventMedia, goToUpdateFormView } = useEventMedias()
...
const confirmDelete = async () => {
  const { success, error } = await deleteEventMedia(selectedEntity.value!)
  if (success) {
