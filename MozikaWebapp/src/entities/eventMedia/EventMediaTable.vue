<template>
  <table class="table table-auto relative">
    ...
    <tbody class="bg-base-100">
      ...
      <component
        v-else
        v-for="eventMedia in data"
        :key="eventMedia.getKeyValue()"
        :is="EventMediaRow"
...
import EventMediaRow from './EventMediaRow.vue'
...
const visibleColCount = computed(
  () =>
    props.visibleFields.filter((f) =>
      EventMedia.getAllSearchFieldsMetadata().some((m) => m.key === f && m.showInTable),
    ).length,
)
</script>
<style scoped></style>
