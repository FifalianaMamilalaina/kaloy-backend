...
import EventModerationStatuseRow from './EventModerationStatuseRow.vue'
...
const visibleColCount = computed(
  () =>
    props.visibleFields.filter((f) =>
      EventModerationStatuse.getAllSearchFieldsMetadata().some((m) => m.key === f && m.showInTable),
    ).length,
)
</script>
<style scoped></style>
