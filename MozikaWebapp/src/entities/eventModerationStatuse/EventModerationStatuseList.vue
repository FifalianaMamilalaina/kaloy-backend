<template>
  ...
  <component
    ...
    @request-view="viewEventModerationStatuse"
...
import EventModerationStatuseTable from './EventModerationStatuseTable.vue'
import EventModerationStatuseGrid from './EventModerationStatuseGrid.vue'
...
const layoutComponents: Record<string, Component> = {
  list: EventModerationStatuseTable,
  card: EventModerationStatuseGrid,
  // Exemples futurs :
  // kanban: EventModerationStatuseKanban,
  // timeline: EventModerationStatuseTimeline,
}

// Fallback sur EventModerationStatuseTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? EventModerationStatuseTable,
)

const selectedEntity = ref<EventModerationStatuse>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deleteEventModerationStatuse, viewEventModerationStatuse, goToUpdateFormView } =
...
const confirmDelete = async () => {
  const { success, error } = await deleteEventModerationStatuse(selectedEntity.value!)
  if (success) {
