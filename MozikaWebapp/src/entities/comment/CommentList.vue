...
import CommentGrid from './CommentGrid.vue'
...
const props = defineProps({
  data: { type: Array as PropType<Comment[]>, required: true },
  message: String,
...
const layoutComponents: Record<string, Component> = {
  ...
  card: CommentGrid,
  // Exemples futurs :
  // kanban: CommentKanban,
  // timeline: CommentTimeline,
}

// Fallback sur CommentTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? CommentTable,
)

const selectedEntity = ref<Comment>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deleteComment, viewComment, goToUpdateFormView } = useComments()
...
const confirmDelete = async () => {
  const { success, error } = await deleteComment(selectedEntity.value!)
  if (success) {
