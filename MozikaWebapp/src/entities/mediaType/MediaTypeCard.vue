  ...
  >
    <div class="card-body p-4">
      ...
      <h2 class="card-title text-base font-bold text-primary mb-2 truncate">
        <span> #{{ mediaTypeData['id'] }} - MediaType </span>
      </h2>

      <!-- Corps de la carte : affiche UNIQUEMENT les champs visibles -->
      <div class="space-y-1 text-sm flex-1">
        <template v-for="field in MediaType.getAllSearchFieldsMetadata()" :key="field.key">
          <div
          ...
          >
            ...
            <span class="text-right text-base-content truncate max-w-[60%]">
              <template v-if="field.type === 'date'">
                {{ formatDate(mediaTypeData[field.key]) }}
              </template>
              <template v-else-if="field.type === 'number'">
                {{
                  mediaTypeData[field.key] != null ? $n(mediaTypeData[field.key], 'decimal') : '--'
                }}
              </template>
              <template v-else-if="field.type === 'select'">
                {{
                  mediaTypeData[field.key] != null ? mediaTypeData[field.key].getKeyValue() : '--'
                }}
              </template>
              <template v-else-if="field.type === 'file'">
                <img
                  v-if="mediaTypeData[field.key]"
                  :src="getUrl(mediaTypeData[field.key])"
                  :alt="field.key"
                  class="max-h-10 max-w-10"
                />
              </template>
              <template v-else>
                {{ mediaTypeData[field.key] ?? '--' }}
              </template>
            </span>
          </div>
        </template>
...
const mediaTypeData = computed(() => {
  return props.mediaType as Record<string, any>
})
...
const deleteRow = () => emit('request-delete', props.mediaType)
const viewRow = () => emit('request-view', props.mediaType)
const updateRow = () => emit('request-update', props.mediaType)
