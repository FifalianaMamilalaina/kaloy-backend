<template>
  ...
  <component
    ...
    @request-view="viewVerificationStatuse"
...
import VerificationStatuseTable from './VerificationStatuseTable.vue'
import VerificationStatuseGrid from './VerificationStatuseGrid.vue'
...
const props = defineProps({
  data: { type: Array as PropType<VerificationStatuse[]>, required: true },
  message: String,
...
const layoutComponents: Record<string, Component> = {
  list: VerificationStatuseTable,
  card: VerificationStatuseGrid,
  // Exemples futurs :
  // kanban: VerificationStatuseKanban,
  // timeline: VerificationStatuseTimeline,
}

// Fallback sur VerificationStatuseTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? VerificationStatuseTable,
)

const selectedEntity = ref<VerificationStatuse>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deleteVerificationStatuse, viewVerificationStatuse, goToUpdateFormView } =
...
const confirmDelete = async () => {
  const { success, error } = await deleteVerificationStatuse(selectedEntity.value!)
  if (success) {
