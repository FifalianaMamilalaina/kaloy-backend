<template>
  <table class="table table-auto relative">
    ...
    <tbody class="bg-base-100">
      ...
      <component
        v-else
        v-for="verificationStatuse in data"
        :key="verificationStatuse.getKeyValue()"
        :is="VerificationStatuseRow"
        :verificationStatuse="verificationStatuse"
        v-bind="itemProps"
...
import VerificationStatuseRow from './VerificationStatuseRow.vue'
...
const visibleColCount = computed(
  () =>
    props.visibleFields.filter((f) =>
      VerificationStatuse.getAllSearchFieldsMetadata().some((m) => m.key === f && m.showInTable),
    ).length,
)
</script>
<style scoped></style>
