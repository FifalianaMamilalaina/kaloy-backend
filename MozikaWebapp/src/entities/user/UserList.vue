...
// Fallback sur UserTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? UserTable,
)

const selectedEntity = ref<User>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deleteUser, viewUser, goToUpdateFormView } = useUsers()
...
const confirmDelete = async () => {
  const { success, error } = await deleteUser(selectedEntity.value!)
  if (success) {
