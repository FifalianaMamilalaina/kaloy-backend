...
import SongGenreTable from './SongGenreTable.vue'
import SongGenreGrid from './SongGenreGrid.vue'
...
const layoutComponents: Record<string, Component> = {
  ...
  card: SongGenreGrid,
  // Exemples futurs :
  // kanban: SongGenreKanban,
  // timeline: SongGenreTimeline,
}

// Fallback sur SongGenreTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? SongGenreTable,
)

const selectedEntity = ref<SongGenre>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deleteSongGenre, viewSongGenre, goToUpdateFormView } = useSongGenres()
...
const confirmDelete = async () => {
  const { success, error } = await deleteSongGenre(selectedEntity.value!)
  if (success) {
