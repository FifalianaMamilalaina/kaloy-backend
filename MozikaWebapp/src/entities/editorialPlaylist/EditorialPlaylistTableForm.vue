<template>
  <div class="w-full bg-base-100 border rounded border-base-300 p-4">
    <h3 class="text-lg font-semibold mb-4 text-gray-700">Editorial playlist</h3>
      ...
      <table class="table w-full">
        ...
        <tbody>
          <template v-for="(row, index) in rows" :key="row._internalId">
            <EditorialPlaylistRowForm
              :internalId="index + 1"
              :editorialPlaylist="row.data"
...
import EditorialPlaylistRowForm from '@/entities/editorialPlaylist/EditorialPlaylistRowForm.vue'
import GenesisInput from '@/components/form/GenesisInput.vue'
import PlusIcon from '@/components/icons/PlusIcon.vue'

const rowToAdd = ref(10)
const props = defineProps<{
  getTableDataCallback: (data: EditorialPlaylistFormDTO[]) => void
  initialData?: EditorialPlaylist[]
