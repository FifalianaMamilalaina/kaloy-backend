...
import EditorialPlaylistRow from './EditorialPlaylistRow.vue'
...
const visibleColCount = computed(
  () =>
    props.visibleFields.filter((f) =>
      EditorialPlaylist.getAllSearchFieldsMetadata().some((m) => m.key === f && m.showInTable),
    ).length,
)
</script>
<style scoped></style>
