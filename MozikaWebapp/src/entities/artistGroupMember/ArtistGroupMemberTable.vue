...
import ArtistGroupMemberRow from './ArtistGroupMemberRow.vue'
...
const visibleColCount = computed(
  () =>
    props.visibleFields.filter((f) =>
      ArtistGroupMember.getAllSearchFieldsMetadata().some((m) => m.key === f && m.showInTable),
    ).length,
)
</script>
<style scoped></style>
