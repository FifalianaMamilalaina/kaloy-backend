<template>
  ...
  <component
    ...
    @request-view="viewArtistGroupMember"
...
import ArtistGroupMemberTable from './ArtistGroupMemberTable.vue'
import ArtistGroupMemberGrid from './ArtistGroupMemberGrid.vue'
...
const layoutComponents: Record<string, Component> = {
  list: ArtistGroupMemberTable,
  card: ArtistGroupMemberGrid,
  // Exemples futurs :
  // kanban: ArtistGroupMemberKanban,
  // timeline: ArtistGroupMemberTimeline,
}

// Fallback sur ArtistGroupMemberTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? ArtistGroupMemberTable,
)

const selectedEntity = ref<ArtistGroupMember>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deleteArtistGroupMember, viewArtistGroupMember, goToUpdateFormView } =
...
const confirmDelete = async () => {
  const { success, error } = await deleteArtistGroupMember(selectedEntity.value!)
  if (success) {
