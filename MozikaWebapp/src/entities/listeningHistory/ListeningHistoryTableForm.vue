<template>
  <div class="w-full bg-base-100 border rounded border-base-300 p-4">
    <h3 class="text-lg font-semibold mb-4 text-gray-700">Listening history</h3>
      ...
      <table class="table w-full">
        <thead>
          <tr class="text-primary-focus">
            <th class="">#</th>
            <th class="">Playmodeid play modes</th>
            <th class="">Listened at</th>
            <th class=""></th>
          </tr>
        </thead>
        <tbody>
          <template v-for="(row, index) in rows" :key="row._internalId">
            <ListeningHistoryRowForm
              :internalId="index + 1"
              :listeningHistory="row.data"
...
import ListeningHistoryRowForm from '@/entities/listeningHistory/ListeningHistoryRowForm.vue'
import GenesisInput from '@/components/form/GenesisInput.vue'
import PlusIcon from '@/components/icons/PlusIcon.vue'

const rowToAdd = ref(10)
const props = defineProps<{
  getTableDataCallback: (data: ListeningHistoryFormDTO[]) => void
  initialData?: ListeningHistory[]
  violations?: Record<string, string>[]
}>()

const { rows, addRow, removeRow, getTableData } = useTableForm<
  ListeningHistory,
  ListeningHistoryFormDTO
>(props.initialData)
