<template>
  <table class="table table-auto relative">
    ...
    <tbody class="bg-base-100">
      ...
      <component
        v-else
        v-for="listeningHistory in data"
        :key="listeningHistory.getKeyValue()"
        :is="ListeningHistoryRow"
...
import ListeningHistoryRow from './ListeningHistoryRow.vue'
...
const visibleColCount = computed(
  () =>
    props.visibleFields.filter((f) =>
      ListeningHistory.getAllSearchFieldsMetadata().some((m) => m.key === f && m.showInTable),
    ).length,
)
</script>
<style scoped></style>
