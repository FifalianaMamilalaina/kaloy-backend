<template>
  <div class="w-full bg-base-100 border rounded border-base-300 p-4">
    <h3 class="text-lg font-semibold mb-4 text-gray-700">User status history</h3>
      ...
      <table class="table w-full">
        <thead>
          <tr class="text-primary-focus">
            <th class="">#</th>
            <th class="">Previousstatusid user statuses</th>
            <th class="">Newstatusid user statuses</th>
            <th class="">Reason</th>
            <th class="">Changedbyuserid users</th>
        ...
        <tbody>
          <template v-for="(row, index) in rows" :key="row._internalId">
            <UserStatusHistoryRowForm
              :internalId="index + 1"
              :userStatusHistory="row.data"
...
import UserStatusHistoryRowForm from '@/entities/userStatusHistory/UserStatusHistoryRowForm.vue'
import GenesisInput from '@/components/form/GenesisInput.vue'
import PlusIcon from '@/components/icons/PlusIcon.vue'

const rowToAdd = ref(10)
const props = defineProps<{
  getTableDataCallback: (data: UserStatusHistoryFormDTO[]) => void
  initialData?: UserStatusHistory[]
