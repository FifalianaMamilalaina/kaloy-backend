<template>
  <table class="table table-auto relative">
    ...
    <tbody class="bg-base-100">
      ...
      <component
        v-else
        v-for="userStatusHistory in data"
        :key="userStatusHistory.getKeyValue()"
        :is="UserStatusHistoryRow"
...
import UserStatusHistoryRow from './UserStatusHistoryRow.vue'
...
const visibleColCount = computed(
  () =>
    props.visibleFields.filter((f) =>
      UserStatusHistory.getAllSearchFieldsMetadata().some((m) => m.key === f && m.showInTable),
    ).length,
)
</script>
<style scoped></style>
