<template>
  ...
  <component
    ...
    @request-view="viewUserStatusHistory"
...
import UserStatusHistoryTable from './UserStatusHistoryTable.vue'
import UserStatusHistoryGrid from './UserStatusHistoryGrid.vue'
...
const props = defineProps({
  data: { type: Array as PropType<UserStatusHistory[]>, required: true },
  message: String,
...
const layoutComponents: Record<string, Component> = {
  list: UserStatusHistoryTable,
  card: UserStatusHistoryGrid,
  // Exemples futurs :
  // kanban: UserStatusHistoryKanban,
  // timeline: UserStatusHistoryTimeline,
}

// Fallback sur UserStatusHistoryTable si le mode n'existe pas (sécurité)
const currentLayoutComponent = computed<Component>(
  () => layoutComponents[props.layoutMode] ?? UserStatusHistoryTable,
)

const selectedEntity = ref<UserStatusHistory>()
const { visible: deletePopup, closePopup } = usePopup(false)
const { deleteUserStatusHistory, viewUserStatusHistory, goToUpdateFormView } =
...
const confirmDelete = async () => {
  const { success, error } = await deleteUserStatusHistory(selectedEntity.value!)
  if (success) {
