<template>
  <table class="table table-auto relative">
    ...
    <tbody class="bg-base-100">
      ...
      <component
        v-else
        v-for="memberStatuse in data"
        :key="memberStatuse.getKeyValue()"
        :is="MemberStatuseRow"
        :memberStatuse="memberStatuse"
        v-bind="itemProps"
...
import MemberStatuseRow from './MemberStatuseRow.vue'
...
const visibleColCount = computed(
  () =>
    props.visibleFields.filter((f) =>
      MemberStatuse.getAllSearchFieldsMetadata().some((m) => m.key === f && m.showInTable),
    ).length,
)
</script>
<style scoped></style>
