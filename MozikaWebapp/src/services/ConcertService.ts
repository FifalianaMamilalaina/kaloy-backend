...
const restApiWithPage = api<IPageResponse<Concert>>()
const restApi = api<Concert>()
const BASE_URL = '/concerts'

/** Helper to wrap response data in Concert instances */
function wrapConcert(response: DataResponse<Concert> | PagedDataResponse<Concert>) {
  if (Array.isArray(response.data)) {
    response.data = response.data.map((r) => new Concert(r))
  } else if (response.data && typeof response.data === 'object') {
    response.data = new Concert(response.data)
  }
  return response
}
...
export async function getAll(
  ...
  return wrapConcert(
    RestResponse.handlePagedResponse<Concert>(response),
  ) as PagedDataResponse<Concert>
}
...
export async function getById(id: number | string | undefined) {
  ...
  return wrapConcert(RestResponse.handleDataResponse<Concert>(response)) as DataResponse<Concert>
}

export async function create(data: Partial<Concert>) {
  const response = await restApi.POST(BASE_URL, data)
  return wrapConcert(RestResponse.handleDataResponse<Concert>(response)) as DataResponse<Concert>
}

/** PUT update */
export async function update(id: number | string | undefined, data: Partial<Concert>) {
...
export async function exportCsv(
  ...
  try {
    ...
    triggerFileDownload(blob, getFilenameFromHeaders(contentDisposition, 'Concert.csv'))
    return {
      success: true,
      error: null,
    }
...
export async function getAllByVenueId(
  id: number | string | undefined,
  pagination?: PaginationRequestParameter,
  sortFields?: SortFieldParameter[],
) {
  if (!id) throw new Error('getAllByVenueId: id of venue is required')
  const request = new RequestModel('/venues/' + id + BASE_URL, pagination, sortFields)
  const response = await restApiWithPage.GET(request.buildRequestUrl())
  return wrapConcert(
    RestResponse.handlePagedResponse<Concert>(response),
  ) as PagedDataResponse<Concert>
