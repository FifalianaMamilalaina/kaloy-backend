...
const restApiWithPage = api<IPageResponse<ArtistGroupMember>>()
const restApi = api<ArtistGroupMember>()
const BASE_URL = '/artistgroupmembers'

/** Helper to wrap response data in ArtistGroupMember instances */
function wrapArtistGroupMember(
  response: DataResponse<ArtistGroupMember> | PagedDataResponse<ArtistGroupMember>,
) {
  if (Array.isArray(response.data)) {
    response.data = response.data.map((r) => new ArtistGroupMember(r))
  } else if (response.data && typeof response.data === 'object') {
    response.data = new ArtistGroupMember(response.data)
  }
  return response
}
...
export async function getAll(
  ...
  return wrapArtistGroupMember(
    RestResponse.handlePagedResponse<ArtistGroupMember>(response),
  ) as PagedDataResponse<ArtistGroupMember>
}

/** POST search with pagination */
export async function search(
  data: Partial<ArtistGroupMember>,
  pagination?: PaginationRequestParameter,
  sortFields?: SortFieldParameter[],
) {
...
export async function getById(id: number | string | undefined) {
  ...
  return wrapArtistGroupMember(
    RestResponse.handleDataResponse<ArtistGroupMember>(response),
  ) as DataResponse<ArtistGroupMember>
}

export async function create(data: Partial<ArtistGroupMember>) {
...
/** PUT update */
export async function update(id: number | string | undefined, data: Partial<ArtistGroupMember>) {
...
export async function exportCsv(
  ...
  try {
    const response = await fetch(url, {
      ...
      body: JSON.stringify(artistGroupMembers),
    })
    if (!response.ok) {
    ...
    triggerFileDownload(blob, getFilenameFromHeaders(contentDisposition, 'ArtistGroupMember.csv'))
    return {
      success: true,
      error: null,
    }
