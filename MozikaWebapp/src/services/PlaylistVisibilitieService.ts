...
const restApiWithPage = api<IPageResponse<PlaylistVisibilitie>>()
const restApi = api<PlaylistVisibilitie>()
const BASE_URL = '/playlistvisibilities'

/** Helper to wrap response data in PlaylistVisibilitie instances */
function wrapPlaylistVisibilitie(
  response: DataResponse<PlaylistVisibilitie> | PagedDataResponse<PlaylistVisibilitie>,
) {
  if (Array.isArray(response.data)) {
    response.data = response.data.map((r) => new PlaylistVisibilitie(r))
  } else if (response.data && typeof response.data === 'object') {
    response.data = new PlaylistVisibilitie(response.data)
  }
  return response
}
...
export async function getAll(
  ...
  return wrapPlaylistVisibilitie(
    RestResponse.handlePagedResponse<PlaylistVisibilitie>(response),
  ) as PagedDataResponse<PlaylistVisibilitie>
}

/** POST search with pagination */
export async function search(
  data: Partial<PlaylistVisibilitie>,
  pagination?: PaginationRequestParameter,
  sortFields?: SortFieldParameter[],
) {
...
export async function getById(id: number | string | undefined) {
  ...
  return wrapPlaylistVisibilitie(
    RestResponse.handleDataResponse<PlaylistVisibilitie>(response),
  ) as DataResponse<PlaylistVisibilitie>
}

export async function create(data: Partial<PlaylistVisibilitie>) {
...
/** PUT update */
export async function update(id: number | string | undefined, data: Partial<PlaylistVisibilitie>) {
...
export async function exportCsv(
  playlistVisibilities: Partial<PlaylistVisibilitie[]>,
  ...
  try {
    const response = await fetch(url, {
      ...
      body: JSON.stringify(playlistVisibilities),
    })
    if (!response.ok) {
    ...
    triggerFileDownload(blob, getFilenameFromHeaders(contentDisposition, 'PlaylistVisibilitie.csv'))
    return {
      success: true,
      error: null,
    }
