...
const restApiWithPage = api<IPageResponse<VerificationChannel>>()
const restApi = api<VerificationChannel>()
const BASE_URL = '/verificationchannels'

/** Helper to wrap response data in VerificationChannel instances */
function wrapVerificationChannel(
  response: DataResponse<VerificationChannel> | PagedDataResponse<VerificationChannel>,
) {
  if (Array.isArray(response.data)) {
    response.data = response.data.map((r) => new VerificationChannel(r))
  } else if (response.data && typeof response.data === 'object') {
    response.data = new VerificationChannel(response.data)
  }
  return response
}
...
export async function getAll(
  ...
  return wrapVerificationChannel(
    RestResponse.handlePagedResponse<VerificationChannel>(response),
  ) as PagedDataResponse<VerificationChannel>
}

/** POST search with pagination */
export async function search(
  data: Partial<VerificationChannel>,
  pagination?: PaginationRequestParameter,
  sortFields?: SortFieldParameter[],
) {
...
export async function getById(id: number | string | undefined) {
  ...
  return wrapVerificationChannel(
    RestResponse.handleDataResponse<VerificationChannel>(response),
  ) as DataResponse<VerificationChannel>
}

export async function create(data: Partial<VerificationChannel>) {
...
/** PUT update */
export async function update(id: number | string | undefined, data: Partial<VerificationChannel>) {
...
export async function exportCsv(
  verificationChannels: Partial<VerificationChannel[]>,
  ...
  try {
    const response = await fetch(url, {
      ...
      body: JSON.stringify(verificationChannels),
    })
    if (!response.ok) {
    ...
    triggerFileDownload(blob, getFilenameFromHeaders(contentDisposition, 'VerificationChannel.csv'))
    return {
      success: true,
      error: null,
    }
