...
const restApiWithPage = api<IPageResponse<UserStatuse>>()
const restApi = api<UserStatuse>()
const BASE_URL = '/userstatuses'

/** Helper to wrap response data in UserStatuse instances */
function wrapUserStatuse(response: DataResponse<UserStatuse> | PagedDataResponse<UserStatuse>) {
  if (Array.isArray(response.data)) {
    response.data = response.data.map((r) => new UserStatuse(r))
  } else if (response.data && typeof response.data === 'object') {
    response.data = new UserStatuse(response.data)
  }
  return response
}
...
export async function getAll(
  ...
  return wrapUserStatuse(
    RestResponse.handlePagedResponse<UserStatuse>(response),
  ) as PagedDataResponse<UserStatuse>
}

/** POST search with pagination */
export async function search(
  data: Partial<UserStatuse>,
  pagination?: PaginationRequestParameter,
  sortFields?: SortFieldParameter[],
) {
...
export async function getById(id: number | string | undefined) {
  ...
  return wrapUserStatuse(
    RestResponse.handleDataResponse<UserStatuse>(response),
  ) as DataResponse<UserStatuse>
}

export async function create(data: Partial<UserStatuse>) {
...
/** PUT update */
export async function update(id: number | string | undefined, data: Partial<UserStatuse>) {
...
export async function exportCsv(
  userStatuses: Partial<UserStatuse[]>,
  ...
  try {
    const response = await fetch(url, {
      ...
      body: JSON.stringify(userStatuses),
    })
    if (!response.ok) {
    ...
    triggerFileDownload(blob, getFilenameFromHeaders(contentDisposition, 'UserStatuse.csv'))
    return {
      success: true,
      error: null,
    }
