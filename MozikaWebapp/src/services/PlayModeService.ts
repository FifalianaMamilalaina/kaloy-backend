...
const restApiWithPage = api<IPageResponse<PlayMode>>()
const restApi = api<PlayMode>()
const BASE_URL = '/playmodes'

/** Helper to wrap response data in PlayMode instances */
function wrapPlayMode(response: DataResponse<PlayMode> | PagedDataResponse<PlayMode>) {
  if (Array.isArray(response.data)) {
    response.data = response.data.map((r) => new PlayMode(r))
  } else if (response.data && typeof response.data === 'object') {
    response.data = new PlayMode(response.data)
  }
  return response
}
...
export async function getAll(
  ...
  return wrapPlayMode(
    RestResponse.handlePagedResponse<PlayMode>(response),
  ) as PagedDataResponse<PlayMode>
}

/** POST search with pagination */
export async function search(
  data: Partial<PlayMode>,
  pagination?: PaginationRequestParameter,
  sortFields?: SortFieldParameter[],
) {
...
export async function getById(id: number | string | undefined) {
  ...
  return wrapPlayMode(RestResponse.handleDataResponse<PlayMode>(response)) as DataResponse<PlayMode>
}

export async function create(data: Partial<PlayMode>) {
  const response = await restApi.POST(BASE_URL, data)
  return wrapPlayMode(RestResponse.handleDataResponse<PlayMode>(response)) as DataResponse<PlayMode>
}

/** PUT update */
export async function update(id: number | string | undefined, data: Partial<PlayMode>) {
...
export async function exportCsv(
  ...
  try {
    ...
    triggerFileDownload(blob, getFilenameFromHeaders(contentDisposition, 'PlayMode.csv'))
    return {
      success: true,
      error: null,
    }
