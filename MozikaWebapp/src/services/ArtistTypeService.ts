...
const restApiWithPage = api<IPageResponse<ArtistType>>()
const restApi = api<ArtistType>()
const BASE_URL = '/artisttypes'

/** Helper to wrap response data in ArtistType instances */
function wrapArtistType(response: DataResponse<ArtistType> | PagedDataResponse<ArtistType>) {
  if (Array.isArray(response.data)) {
    response.data = response.data.map((r) => new ArtistType(r))
  } else if (response.data && typeof response.data === 'object') {
    response.data = new ArtistType(response.data)
  }
  return response
}
...
export async function getAll(
  ...
  return wrapArtistType(
    RestResponse.handlePagedResponse<ArtistType>(response),
  ) as PagedDataResponse<ArtistType>
}
...
export async function getById(id: number | string | undefined) {
  ...
  return wrapArtistType(
    RestResponse.handleDataResponse<ArtistType>(response),
  ) as DataResponse<ArtistType>
}

export async function create(data: Partial<ArtistType>) {
...
/** PUT update */
export async function update(id: number | string | undefined, data: Partial<ArtistType>) {
...
export async function exportCsv(
  artistTypes: Partial<ArtistType[]>,
  ...
  try {
    const response = await fetch(url, {
      ...
      body: JSON.stringify(artistTypes),
    })
    if (!response.ok) {
    ...
    triggerFileDownload(blob, getFilenameFromHeaders(contentDisposition, 'ArtistType.csv'))
    return {
      success: true,
      error: null,
    }
