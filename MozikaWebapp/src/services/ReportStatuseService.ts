...
const restApiWithPage = api<IPageResponse<ReportStatuse>>()
const restApi = api<ReportStatuse>()
const BASE_URL = '/reportstatuses'

/** Helper to wrap response data in ReportStatuse instances */
function wrapReportStatuse(
  response: DataResponse<ReportStatuse> | PagedDataResponse<ReportStatuse>,
) {
  if (Array.isArray(response.data)) {
    response.data = response.data.map((r) => new ReportStatuse(r))
  } else if (response.data && typeof response.data === 'object') {
    response.data = new ReportStatuse(response.data)
  }
  return response
}
...
export async function getAll(
  ...
  return wrapReportStatuse(
    RestResponse.handlePagedResponse<ReportStatuse>(response),
  ) as PagedDataResponse<ReportStatuse>
}

/** POST search with pagination */
export async function search(
  data: Partial<ReportStatuse>,
  pagination?: PaginationRequestParameter,
  sortFields?: SortFieldParameter[],
) {
...
export async function getById(id: number | string | undefined) {
  ...
  return wrapReportStatuse(
    RestResponse.handleDataResponse<ReportStatuse>(response),
  ) as DataResponse<ReportStatuse>
}

export async function create(data: Partial<ReportStatuse>) {
...
/** PUT update */
export async function update(id: number | string | undefined, data: Partial<ReportStatuse>) {
...
export async function exportCsv(
  reportStatuses: Partial<ReportStatuse[]>,
  ...
  try {
    const response = await fetch(url, {
      ...
      body: JSON.stringify(reportStatuses),
    })
    if (!response.ok) {
    ...
    triggerFileDownload(blob, getFilenameFromHeaders(contentDisposition, 'ReportStatuse.csv'))
    return {
      success: true,
      error: null,
    }
