...
const restApiWithPage = api<IPageResponse<PlaylistSong>>()
const restApi = api<PlaylistSong>()
const BASE_URL = '/playlistsongs'

/** Helper to wrap response data in PlaylistSong instances */
function wrapPlaylistSong(response: DataResponse<PlaylistSong> | PagedDataResponse<PlaylistSong>) {
  if (Array.isArray(response.data)) {
    response.data = response.data.map((r) => new PlaylistSong(r))
  } else if (response.data && typeof response.data === 'object') {
    response.data = new PlaylistSong(response.data)
  }
  return response
}
...
export async function getAll(
  ...
  return wrapPlaylistSong(
    RestResponse.handlePagedResponse<PlaylistSong>(response),
  ) as PagedDataResponse<PlaylistSong>
}

/** POST search with pagination */
export async function search(
  data: Partial<PlaylistSong>,
  pagination?: PaginationRequestParameter,
  sortFields?: SortFieldParameter[],
) {
...
export async function getById(id: number | string | undefined) {
  ...
  return wrapPlaylistSong(
    RestResponse.handleDataResponse<PlaylistSong>(response),
  ) as DataResponse<PlaylistSong>
}

export async function create(data: Partial<PlaylistSong>) {
...
/** PUT update */
export async function update(id: number | string | undefined, data: Partial<PlaylistSong>) {
...
export async function exportCsv(
  ...
  try {
    ...
    triggerFileDownload(blob, getFilenameFromHeaders(contentDisposition, 'PlaylistSong.csv'))
    return {
      success: true,
      error: null,
    }
