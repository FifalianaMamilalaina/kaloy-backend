...
const restApiWithPage = api<IPageResponse<NotificationType>>()
const restApi = api<NotificationType>()
const BASE_URL = '/notificationtypes'

/** Helper to wrap response data in NotificationType instances */
function wrapNotificationType(
  response: DataResponse<NotificationType> | PagedDataResponse<NotificationType>,
) {
  if (Array.isArray(response.data)) {
    response.data = response.data.map((r) => new NotificationType(r))
  } else if (response.data && typeof response.data === 'object') {
    response.data = new NotificationType(response.data)
  }
  return response
}
...
export async function getAll(
  ...
  return wrapNotificationType(
    RestResponse.handlePagedResponse<NotificationType>(response),
  ) as PagedDataResponse<NotificationType>
}
...
export async function getById(id: number | string | undefined) {
  ...
  return wrapNotificationType(
    RestResponse.handleDataResponse<NotificationType>(response),
  ) as DataResponse<NotificationType>
}

export async function create(data: Partial<NotificationType>) {
...
/** PUT update */
export async function update(id: number | string | undefined, data: Partial<NotificationType>) {
...
export async function exportCsv(
  notificationTypes: Partial<NotificationType[]>,
  ...
  try {
    ...
    triggerFileDownload(blob, getFilenameFromHeaders(contentDisposition, 'NotificationType.csv'))
    return {
      success: true,
      error: null,
    }
