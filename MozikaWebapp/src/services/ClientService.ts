...
const restApiWithPage = api<IPageResponse<Client>>()
const restApi = api<Client>()
const BASE_URL = '/clients'

/** Helper to wrap response data in Client instances */
function wrapClient(response: DataResponse<Client> | PagedDataResponse<Client>) {
  if (Array.isArray(response.data)) {
    response.data = response.data.map((r) => new Client(r))
...
export async function getAll(
  ...
  return wrapClient(RestResponse.handlePagedResponse<Client>(response)) as PagedDataResponse<Client>
}
...
export async function getById(id: number | string | undefined) {
  ...
  return wrapClient(RestResponse.handleDataResponse<Client>(response)) as DataResponse<Client>
}

export async function create(data: Partial<Client>) {
  const response = await restApi.POST(BASE_URL, data)
  return wrapClient(RestResponse.handleDataResponse<Client>(response)) as DataResponse<Client>
}

/** PUT update */
export async function update(id: number | string | undefined, data: Partial<Client>) {
...
export async function exportCsv(
  ...
  try {
    ...
    triggerFileDownload(blob, getFilenameFromHeaders(contentDisposition, 'Client.csv'))
    return {
      success: true,
      error: null,
    }
