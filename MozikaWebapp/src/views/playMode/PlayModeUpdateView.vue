  ...
  <div class="w-full">
    ...
    <div v-if="entity">
      <playMode-form
        ...
        :submit-label="$t('entity.update.submitLabel', { entity: 'PlayMode' })"
...
const { getPlayModeById, goToListView, updatePlayMode, viewPlayMode, message } = usePlayModes()
const entity = ref<PlayMode | null>(null)
const freezeStore = useFreezeScreenStore()

const violations = ref<Record<string, object> | null>(null)

const updateHandler = async (formDTO: Partial<PlayModeFormDTO>) => {
  freezeStore.freeze('Updating playMode ' + pathId + ' ...')
  try {
    const data = await updatePlayMode(pathId, formDTO)
    if (data && !message.value) viewPlayMode(data)
