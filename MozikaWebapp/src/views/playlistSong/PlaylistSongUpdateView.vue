  ...
  <div class="w-full">
    ...
    <div v-if="entity">
      <playlistSong-form
        ...
        :submit-label="$t('entity.update.submitLabel', { entity: 'PlaylistSong' })"
...
const { getPlaylistSongById, goToListView, updatePlaylistSong, viewPlaylistSong, message } =
  usePlaylistSongs()
...
const updateHandler = async (formDTO: Partial<PlaylistSongFormDTO>) => {
  freezeStore.freeze('Updating playlistSong ' + pathId + ' ...')
  try {
    const data = await updatePlaylistSong(pathId, formDTO)
    if (data && !message.value) viewPlaylistSong(data)
