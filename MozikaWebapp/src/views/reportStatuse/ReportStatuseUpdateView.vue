<template>
  <div class="w-full">
    ...
    <div v-if="entity">
      <reportStatuse-form
        :reportStatuse="entity"
        :violations="violations"
        :submit-label="$t('entity.update.submitLabel', { entity: 'ReportStatuse' })"
...
const { getReportStatuseById, goToListView, updateReportStatuse, viewReportStatuse, message } =
  useReportStatuses()
...
const updateHandler = async (formDTO: Partial<ReportStatuseFormDTO>) => {
  freezeStore.freeze('Updating reportStatuse ' + pathId + ' ...')
  try {
    const data = await updateReportStatuse(pathId, formDTO)
    if (data && !message.value) viewReportStatuse(data)
...
const cancelHandler = () => {
  if (entity.value) {
    viewReportStatuse(entity.value)
  }
}
