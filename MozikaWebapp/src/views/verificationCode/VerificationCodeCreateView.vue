<template>
  <div class="w-full">
    ...
    <verificationCode-form
      :submit-label="$t('entity.create.submitLabel', { entity: 'VerificationCode' })"
...
const { createVerificationCode, goToListView, message, viewVerificationCode } =
...
const createHandler = async (formDTO: Partial<VerificationCodeFormDTO>) => {
  freezeStore.freeze('Creating a new VerificationCode ...')
  try {
    const { data, errors } = await createVerificationCode(formDTO)
    if (errors) {
      violations.value = errors
    }
    if (data) {
      viewVerificationCode(data)
      return
    }
    throw new Error(message.value || 'Creation failed without an error message from the API.')
  } catch (error: unknown) {
    console.error('Error creating verificationCode:', error)
    openPopup()
  } finally {
