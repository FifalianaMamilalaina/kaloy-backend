<template>
  <div class="w-full">
    ...
    <verificationChannel-form
      :submit-label="$t('entity.create.submitLabel', { entity: 'VerificationChannel' })"
...
import { VerificationChannelFormDTO } from '@/models/VerificationChannelModel'
...
const { createVerificationChannel, goToListView, message, viewVerificationChannel } =
...
const createHandler = async (formDTO: Partial<VerificationChannelFormDTO>) => {
  freezeStore.freeze('Creating a new VerificationChannel ...')
  try {
    const { data, errors } = await createVerificationChannel(formDTO)
    if (errors) {
      violations.value = errors
    }
    if (data) {
      viewVerificationChannel(data)
      return
    }
    throw new Error(message.value || 'Creation failed without an error message from the API.')
  } catch (error: unknown) {
    console.error('Error creating verificationChannel:', error)
    openPopup()
  } finally {
