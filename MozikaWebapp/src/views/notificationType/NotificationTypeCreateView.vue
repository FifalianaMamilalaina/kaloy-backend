<template>
  <div class="w-full">
    ...
    <notificationType-form
      :submit-label="$t('entity.create.submitLabel', { entity: 'NotificationType' })"
...
import { NotificationTypeFormDTO } from '@/models/NotificationTypeModel'
...
const { createNotificationType, goToListView, message, viewNotificationType } =
  useNotificationTypes()
const { openPopup, closePopup, visible: alertPopup } = usePopup()
const freezeStore = useFreezeScreenStore()
const violations = ref<Record<string, string> | null>(null)
const createHandler = async (formDTO: Partial<NotificationTypeFormDTO>) => {
  freezeStore.freeze('Creating a new NotificationType ...')
  try {
    const { data, errors } = await createNotificationType(formDTO)
    if (errors) {
    ...
    console.error('Error creating notificationType:', error)
    openPopup()
  } finally {
