<template>
  <div class="w-full">
    ...
    <div v-if="entity">
      <memberStatuse-form
        :memberStatuse="entity"
        :violations="violations"
        :submit-label="$t('entity.update.submitLabel', { entity: 'MemberStatuse' })"
...
const { getMemberStatuseById, goToListView, updateMemberStatuse, viewMemberStatuse, message } =
  useMemberStatuses()
...
const updateHandler = async (formDTO: Partial<MemberStatuseFormDTO>) => {
  freezeStore.freeze('Updating memberStatuse ' + pathId + ' ...')
  try {
    const data = await updateMemberStatuse(pathId, formDTO)
    if (data && !message.value) viewMemberStatuse(data)
...
const cancelHandler = () => {
  if (entity.value) {
    viewMemberStatuse(entity.value)
  }
}
