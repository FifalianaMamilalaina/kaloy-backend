  ...
  <div class="w-full">
    ...
    <div v-if="entity">
      <searchHistory-form
        ...
        :submit-label="$t('entity.update.submitLabel', { entity: 'SearchHistory' })"
...
const { getSearchHistoryById, goToListView, updateSearchHistory, viewSearchHistory, message } =
  useSearchHistorys()
...
const updateHandler = async (formDTO: Partial<SearchHistoryFormDTO>) => {
  freezeStore.freeze('Updating searchHistory ' + pathId + ' ...')
  try {
    const data = await updateSearchHistory(pathId, formDTO)
    if (data && !message.value) viewSearchHistory(data)
...
const cancelHandler = () => {
  if (entity.value) {
    viewSearchHistory(entity.value)
  }
}
