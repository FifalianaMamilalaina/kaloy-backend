  ...
  <div class="w-full">
    ...
    <div v-if="entity">
      <artistType-form
        ...
        :submit-label="$t('entity.update.submitLabel', { entity: 'ArtistType' })"
...
const { getArtistTypeById, goToListView, updateArtistType, viewArtistType, message } =
  useArtistTypes()
...
const updateHandler = async (formDTO: Partial<ArtistTypeFormDTO>) => {
  freezeStore.freeze('Updating artistType ' + pathId + ' ...')
  try {
    const data = await updateArtistType(pathId, formDTO)
    if (data && !message.value) viewArtistType(data)
