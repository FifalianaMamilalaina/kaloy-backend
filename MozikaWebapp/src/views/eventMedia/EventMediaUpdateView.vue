  ...
  <div class="w-full">
    ...
    <div v-if="entity">
      <eventMedia-form
        ...
        :submit-label="$t('entity.update.submitLabel', { entity: 'EventMedia' })"
...
const { getEventMediaById, goToListView, updateEventMedia, viewEventMedia, message } =
  useEventMedias()
...
const updateHandler = async (formDTO: Partial<EventMediaFormDTO>) => {
  freezeStore.freeze('Updating eventMedia ' + pathId + ' ...')
  try {
    const data = await updateEventMedia(pathId, formDTO)
    if (data && !message.value) viewEventMedia(data)
