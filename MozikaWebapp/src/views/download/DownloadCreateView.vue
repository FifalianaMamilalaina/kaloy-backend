<template>
  <div class="w-full">
    ...
    <download-form
      :submit-label="$t('entity.create.submitLabel', { entity: 'Download' })"
...
const { createDownload, goToListView, message, viewDownload } = useDownloads()
const { openPopup, closePopup, visible: alertPopup } = usePopup()
const freezeStore = useFreezeScreenStore()
const violations = ref<Record<string, string> | null>(null)
const createHandler = async (formDTO: Partial<DownloadFormDTO>) => {
  freezeStore.freeze('Creating a new Download ...')
  try {
    const { data, errors } = await createDownload(formDTO)
    if (errors) {
