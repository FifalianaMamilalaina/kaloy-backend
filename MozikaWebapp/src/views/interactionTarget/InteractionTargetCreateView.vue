<template>
  <div class="w-full">
    ...
    <interactionTarget-form
      :submit-label="$t('entity.create.submitLabel', { entity: 'InteractionTarget' })"
...
import { InteractionTargetFormDTO } from '@/models/InteractionTargetModel'
...
const { createInteractionTarget, goToListView, message, viewInteractionTarget } =
...
const createHandler = async (formDTO: Partial<InteractionTargetFormDTO>) => {
  freezeStore.freeze('Creating a new InteractionTarget ...')
  try {
    const { data, errors } = await createInteractionTarget(formDTO)
    if (errors) {
      violations.value = errors
    }
    if (data) {
      viewInteractionTarget(data)
      return
    }
    throw new Error(message.value || 'Creation failed without an error message from the API.')
  } catch (error: unknown) {
    console.error('Error creating interactionTarget:', error)
    openPopup()
  } finally {
